...
class SurfacePointCloud:
    ...
    def get_sdf(
        ...
        if use_depth_buffer:
            ...
            # TODO: check new implementation above is same as the old below
            # note to self: it looks like DISSM never uses depth buffer, so this is untested
            # old dissm implementation:
    ...
    def sample_sdf_near_surface(
        ...
        query_points.append(
            surface_points
            + np.random.normal(scale=jitter, size=(surface_sample_count, 3))
        )
        query_points.append(
            surface_points
            + np.random.normal(scale=jitter * 0.1, size=(surface_sample_count, 3))
        )
